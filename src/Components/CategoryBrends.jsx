...
function CategoryBrends() {
  ...
  const category = Umico.filter((product) => product.Logo == item);

  const retailPrices = category.map((product) => product.retail_price);
  ...
  const [filterContent, setFilterContent] = useState(category);

  const handlePriceFilter = () => {
    const PriceFilter = category.filter(
      (product) =>
  ...
  const totalPages = Math.ceil(filterContent.length / productsPerPage);
  // sebet
    ...
    <div className="category-page-background">
      <div className="category-page">
        ...
        <div className="category-page-left-category">
          ...
          {totalPages > 1 && filterContent.length > productsPerPage && (
